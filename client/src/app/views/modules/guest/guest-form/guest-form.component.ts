...
export class GuestFormComponent extends AbstractComponent implements OnInit {
  ...
  async submit(): Promise<void> {
    ...
    guest.nametitle = this.nametitleField.value;
    guest.callingname = this.callingnameField.value;
    guest.civilstatus = this.civilstatusField.value;
    guest.fullname = this.fullnameField.value;
    const photoIds = this.photoField.value;
    if (photoIds !== null && photoIds !== []){
      guest.photo = photoIds[0];
    }else{
      guest.photo = null;
    }
    guest.dobirth = DateHelper.getDateAsString(this.dobirthField.value);
    guest.gender = this.genderField.value;
    guest.nic = this.nicField.value;
    guest.mobile = this.mobileField.value;
    guest.land = this.landField.value;
    guest.email = this.emailField.value;
    guest.address = this.addressField.value;
    guest.designation = this.designationField.value;
    guest.dorecruit = this.dorecruitField.value ? DateHelper.getDateAsString(this.dorecruitField.value) : null;
    guest.description = this.descriptionField.value;
    try{
      const resourceLink: ResourceLink = await this.guestService.add(guest);
